...
def make_predict(report_text):
    ...
    russian_stopwords = ['и', 'в', 'во', 'не', 'что','он', 'на', 'я', 'с', 'со', 'как', 'а', 'то', 'все', 'она', 'так', 'его', 'но', 'да', 'ты',
     ...
     'здесь', 'этом', 'один', 'почти', 'мой', 'тем', 'чтобы', 'нее', 'сейчас', 'были', 'куда', 'зачем', 'всех', 'никогда', 'можно', 'при',
     ...
     'такой', 'им', 'более', 'всегда', 'конечно', 'всю', 'между', 'это', 'также', 'данный', 'слово', 'который']
    no_features = 40
    ...
    result_string_category = ''
    if pr == 0:
        result_string_category = 'Жалоба общего характера'
    if pr == 1:
        result_string_category = 'Пункты забора анализов'
    if pr == 1:
        result_string_category = 'Качество обслуживания'
    return result_string_category
